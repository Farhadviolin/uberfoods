...
export const MealPlannerManagement = React.memo(function MealPlannerManagement() {
  ...
  const [newPlan, setNewPlan] = useState({ title: '', restaurantId: '', dishIds: '', notes: '' });
  ...
  const loadWeeklyPlan = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get<{ meals?: MealPlan[] } | MealPlan[]>(`/meal-planner/weekly`, { params: { weekStart } });
  ...
  const fetchShoppingList = useCallback(async () => {
    ...
    try {
      const res = await api.get(`/meal-planner/shopping-list`, {
        params: { startDate: shoppingRange.start, endDate: shoppingRange.end },
      });
      showToast(`Einkaufsliste geladen (${(res.data?.items || []).length || 0} Einträge)`, 'success');
    } catch (error) {
      showToast(extractErrorMessage(error), 'error');
    }
  }, [shoppingRange, showToast]);

  const createMealPlan = useCallback(async (e: FormEvent) => {
    e.preventDefault();
    ...
    const dishIds = newPlan.dishIds.split(',').map((d) => d.trim()).filter(Boolean);
    if (dishIds.length === 0) {
      showToast('Bitte geben Sie gültige Dish IDs ein', 'error');
      return;
    }
    try {
      await api.post('/meal-planner/meals', {
  ...
  const executePlan = useCallback(async () => {
    if (!executeId) return;
    try {
      await api.post(`/meal-planner/meals/${executeId}/execute`);
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h2>Meal Planner</h2>
        <p className="text-muted">Meal-Pläne anlegen, Woche abrufen, Einkaufsliste generieren.</p>
      </div>
      ...
      <div className="grid three-cols gap card-section">
        <div>
          ...
          />
          <button type="button" className="btn" onClick={loadWeeklyPlan} aria-label="Weekly Plan laden">Weekly laden</button>
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) => setShoppingRange((prev) => ({ ...prev, start: e.target.value }))}
            placeholder="2025-01-06"
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setShoppingRange((prev) => ({ ...prev, end: e.target.value }))}
            placeholder="2025-01-12"
            aria-label="Enddatum für Einkaufsliste"
          />
          <button type="button" className="btn" onClick={fetchShoppingList} aria-label="Einkaufsliste abrufen">Einkaufsliste abrufen</button>
        </div>
      </div>

      <form className="card-section" onSubmit={createMealPlan} aria-label="Meal-Plan anlegen">
        ...
        <input
          ...
          onChange={(e) => setNewPlan((prev) => ({ ...prev, restaurantId: e.target.value }))}
        ...
        <input
          ...
          onChange={(e) => setNewPlan((prev) => ({ ...prev, dishIds: e.target.value }))}
        ...
        />
        <button type="submit" className="btn primary" aria-label="Meal-Plan speichern">Speichern</button>
      </form>
      ...
      <div className="card-section">
        ...
        {plans.length === 0 ? (
          <EmptyState
            message={weekStart ? "Keine Pläne für diese Woche gefunden. Erstellen Sie einen neuen Meal-Plan." : "Wählen Sie eine Woche aus, um Pläne zu laden."}
            icon="📋"
            action={weekStart ? {
              label: "Meal-Plan erstellen",
              onClick: () => {
                const form = document.querySelector('form[aria-label="Meal-Plan anlegen"]');
            ...
            {plans.map((plan) => (
              <div key={plan.id} className="card nested">
                <div className="card-header-row">
                  ...
                  <button type="button" className="btn" onClick={() => setExecuteId(plan.id)}>
                    Ausführen vorbereiten
                  </button>
                </div>
                {plan.notes && <p className="text-muted">{plan.notes}</p>}
                {plan.dishIds && (
                  <p className="text-muted">Dishes: {plan.dishIds.join(', ')}</p>
                )}
              </div>
            ))}
          </div>
