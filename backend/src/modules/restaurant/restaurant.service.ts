...
export class RestaurantService {
  ...
  async getCurrentOperatingHours(id: string) {
    ...
    return {
      isOpen: !hours?.[dayOfWeek]?.isClosed,
  ...
  async getCapacity(id: string) {
    ...
    return {
      ...
      maxCapacity: (settings?.maxCapacity as number) || 100,
      availableCapacity:
        ((settings?.maxCapacity as number) || 100) -
        ((settings?.currentCapacity as number) || 0),
    };
  }
  ...
  async getCurrentCapacity(id: string) {
    ...
    const restaurant = await this.findOne(id);
    const settings = (restaurant.settings as RestaurantSettings) || {};
    const maxCapacity = (settings?.maxCapacity as number) || 100;

    return {
      currentCapacity: activeOrders as number,
      maxCapacity,
      availableCapacity: Math.max(0, maxCapacity - (activeOrders as number)),
    ...
    return this.prisma.restaurant.update({
      ...
      data: {
        settings: {
          ...
          currentCapacity:
            (data.currentCapacity as number) ?? settings.currentCapacity,
        } as any,
      },
    });
  }
  ...
  async reserveCapacity(id: string, amount: number = 1) {
    ...
    return this.prisma.restaurant.update({
      ...
      data: {
        settings: {
          ...settings,
          currentCapacity: ((settings.currentCapacity as number) || 0) + amount,
        },
      },
    });
  }
