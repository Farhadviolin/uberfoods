...
export class RestaurantController {
  ...
  @Get(":id/dishes")
  @ApiOperation({ summary: "Get restaurant dishes" })
  @ApiResponse({ status: 200, description: "Dishes retrieved successfully" })
  async getRestaurantDishes(@Param("id") id: string) {
  ...
  async update(@Param("id") id: string, @Body() data: UpdateRestaurantDto) {
    // Transform operatingHours from array to object format expected by service
    const transformedData = { ...data };
    if (data.operatingHours) {
      transformedData.operatingHours = data.operatingHours.reduce(
        (acc, hour) => {
          acc[hour.day] = {
            open: hour.open,
            close: hour.close,
    ...
    return this.restaurantService.update(id, transformedData as any);
  }
  ...
  async exportReport(
    ...
    if (format === "csv") {
      ...
      const csv = this.convertToCSV(reportData as any);
      return res.send(csv);
    ...
    const flatten = (obj: CSVData, prefix = ""): CSVData => {
      ...
      for (const key in obj) {
        if (Array.isArray(obj[key])) {
          ...
          Object.assign(result, flatten(obj[key] as any, `${prefix}${key}.`));
        } else {
