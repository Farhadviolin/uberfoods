...
export class HealthController {
  ...
  @Get()
  @ApiOperation({ summary: "Health Check - Systemstatus prüfen" })
  ...
  @Get("ready")
  @ApiOperation({ summary: "Readiness Check - Bereit für Traffic" })
  @ApiResponse({ status: 200, description: "Service ist bereit für Traffic" })
  @ApiResponse({ status: 503, description: "Service ist nicht bereit" })
  async readiness() {
    try {
      ...
      // This is a simplified approach - in production you'd use proper HTTP status
      // For now, we rely on the status field in the response
      return response;
    }
  }

  @Get("live")
  @ApiOperation({ summary: "Liveness Check - Service läuft" })
  async liveness() {
    return {
      status: "alive",
      timestamp: new Date().toISOString(),
  ...
  @Get("debug/routes")
  @ApiOperation({ summary: "Debug: Show loaded routes and controllers" })
  @ApiResponse({ status: 200, description: "Route information" })
  getRoutes() {
    // Simple debug info - can't use DiscoveryService without proper setup
    return {
      globalPrefix: "api",
      message:
        "Debug endpoint working - check backend logs for controller loading",
      note: "DriverController should appear in backend startup logs if loaded",
    };
  }
}
